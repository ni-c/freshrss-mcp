...
Five of the sixteen tools change something that FreshRSS keeps no record of
undoing. All five **ask a person first**.
...
| Tool | When it asks |
| --- | --- |
| `mark_all_as_read` | always |
| `unsubscribe_feed` | always |
| `delete_category_or_label` | always |
| `import_opml` | always |
| `mark_articles` | only when it is about to mark something **read** |
| everything else | never |

The `mark_articles` line is the one worth explaining. Starring, unstarring,
labelling and marking *unread* all set a field that can be set back, and asking
about a star toggle would be how people learn to tick without reading. Marking
read destroys **which** of those articles were unread, and FreshRSS keeps no
record of that — the same reason `mark_all_as_read` is guarded, over a
caller-named list instead of a whole stream.

Its description used to say the opposite in so many words: “All changes are
reversible by calling this tool again with the opposite value.” Three of the
four are.

## What the dialog contains

Ids and counts. Never a feed title, an article headline or a category name.

That is not a formatting preference. Everything FreshRSS hands back was written
by a stranger who can publish to a feed, and the prompt is read by a model at the
exact moment it is deciding whether to do something irreversible. Even the
description of a category resolves to the constant “the selected category”
rather than its name.

```
This will mark 12 article(s) as read.

FreshRSS keeps no record of which of them were unread, so this cannot be undone.
The other changes in the same call — stars and labels — can be set back.
```
...
`ELICITATION` has no `FRESHRSS_` in front of it, so one
...
- a server started with it off prints one line at startup, in the log of every
  ...
  ```
  freshrss-mcp: ELICITATION=false — guarded tools fall back to the two-call token
  ```
...
`update_feed` and `rename_category_or_label` are exactly that case: they replace
a name somebody chose, and they are not asked about.
