...
- `FRESHRSS_READ_ONLY` is set, and it is a write tool.
- `FRESHRSS_ALLOW_TOOLS` is set and does not name it — it is an allow list, so
  anything not named is out.
- `FRESHRSS_DENY_TOOLS` names it, possibly through a prefix such as `list_*`.
