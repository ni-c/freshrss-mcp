...
export function buildToolFilter(config: {
  ...
  if (allow === undefined) {
    ...
    for (const entry of allow) {
      ...
      const matches = expand(entry, 'FRESHRSS_ALLOW_TOOLS');
      if (matches.length === 0) {
        throw new ToolFilterError(
          `FRESHRSS_ALLOW_TOOLS: no tool matches "${entry}". ` +
      ...
      if (survivors.length === 0) {
        if (entry.endsWith('*')) {
          ...
          console.error(
            `freshrss-mcp: FRESHRSS_ALLOW_TOOLS: "${entry}" matches only write ` +
              'tools, which FRESHRSS_READ_ONLY suppresses — it contributes nothing.'
          );
        ...
        throw new ToolFilterError(
          `FRESHRSS_ALLOW_TOOLS: "${entry}" is a write tool, but FRESHRSS_READ_ONLY ` +
            'is set — it is never registered. Remove it from FRESHRSS_ALLOW_TOOLS, ' +
            `or unset FRESHRSS_READ_ONLY. Available in read-only mode: ${[...READ_TOOLS].sort().join(', ')}.`
        );
      }
  ...
  for (const entry of deny ?? []) {
    ...
    const matches = expand(entry, 'FRESHRSS_DENY_TOOLS');
    if (matches.length === 0) {
      throw new ToolFilterError(
        `FRESHRSS_DENY_TOOLS: no tool matches "${entry}". Valid tools: ${catalogueList()}.`
      );
    }
  ...
  if (selected.size === 0) {
    throw new ToolFilterError(
      suppressedByReadOnly
        ? 'FRESHRSS_ALLOW_TOOLS selects only write tools, but FRESHRSS_READ_ONLY is ' +
            'set — the server would start with an empty tool list.'
        : 'FRESHRSS_ALLOW_TOOLS/FRESHRSS_DENY_TOOLS leave no tools registered — the ' +
