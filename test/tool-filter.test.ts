...
describe('selecting tools', () => {
  it('narrows tools/list to an allow list', async () => {
    expect(await toolNames({ allowTools: 'export_opml,get_articles' })).toEqual(
      ['export_opml', 'get_articles'].sort()
    );
  });

  it('removes a whole family with a prefix pattern', async () => {
    const names = await toolNames({ denyTools: 'list_*' });
    expect(names.some((n) => n.startsWith('list_'))).toBe(false);
  ...
  it('subtracts the deny list from the allow list', async () => {
    expect(
      await toolNames({
        allowTools: 'export_opml,get_articles',
        denyTools: 'get_articles',
      })
    ).toEqual(['export_opml']);
  });
  ...
  it('lets the preset compose with extra names', async () => {
    expect(
      await toolNames({ allowTools: 'essential,delete_category_or_label' })
    ).toEqual([...ESSENTIAL_TOOLS, 'delete_category_or_label'].sort());
  });

  it('trims entries, ignores case and skips empty ones', async () => {
    expect(
      await toolNames({ allowTools: ' EXPORT_OPML ,, get_articles, ' })
    ).toEqual(['export_opml', 'get_articles'].sort());
  });
...
describe('a filtered-out tool', () => {
  it('cannot be called either, not merely hidden', async () => {
    ...
    const server = createServer(config({ allowTools: 'export_opml' }));
    const client = new Client({ name: 'test-client', version: '0.0.0' });
    const [clientTransport, serverTransport] =
      InMemoryTransport.createLinkedPair();
    await Promise.all([
    ...
    ).rejects.toThrow('Tool delete_category_or_label not found');
...
describe('refusing an unusable list', () => {
  it('rejects a name no tool has, and says which names exist', () => {
    ...
    expect(() => createServer(config({ allowTools: 'export_opmz' }))).toThrow(
      ToolFilterError
    );
    expect(() => createServer(config({ allowTools: 'export_opmz' }))).toThrow(
      /no tool matches "export_opmz".*export_opml/s
    );
  });
  ...
  it('applies the same rule to the deny list', () => {
    expect(() => createServer(config({ denyTools: 'export_opmz' }))).toThrow(
      /_DENY_TOOLS: no tool matches "export_opmz"/
    );
  });
...
describe('together with read-only mode', () => {
  ...
  it('names read-only as the reason, rather than calling the tool unknown', () => {
    ...
    try {
      createServer(
        config({ ...readOnly, allowTools: 'delete_category_or_label' })
      );
    } catch (error) {
      thrown = error;
    }
  ...
  it('does not apply the write-tool rule to the deny list', async () => {
    ...
    expect(
      await toolNames({ ...readOnly, denyTools: 'delete_category_or_label' })
  ...
  it('lets a pattern cover write tools without failing', async () => {
    ...
    expect(
      await toolNames({ ...readOnly, allowTools: 'essential,delete_*' })
    ).toEqual(
      ESSENTIAL_TOOLS.filter((t) =>
        (READ_TOOLS as readonly string[]).includes(t)
      ).sort()
  ...
  it('says read-only is the reason when a pattern leaves nothing at all', () => {
    ...
    expect(() =>
      createServer(config({ ...readOnly, allowTools: 'delete_*' }))
